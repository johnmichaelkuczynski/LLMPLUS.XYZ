...
**Why:** The workspace files were restored, but the Node server continued executing the later broken code already loaded in memory. The app looked unchanged until the workflow restarted; the user then confirmed the recovered behavior was much better.
