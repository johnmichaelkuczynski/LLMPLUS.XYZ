...
description: server/index.js is ESM; require() throws at runtime even though much of the file uses CommonJS-style syntax.
---
- server/index.js LOOKS like CommonJS (var, function-style) but is ESM (it uses `import`). `require('x')` inside handlers throws ReferenceError at runtime — silently, if inside a try/catch. Use imports or globals (e.g. `globalThis.crypto.randomUUID()`). `node --check` will NOT catch this.
**Why:** a visitor-tracking middleware silently no-oped and an admin endpoint always 401'd because of these two traps.
**How to apply:** any new server middleware/route — never use require() inside this ESM file.
