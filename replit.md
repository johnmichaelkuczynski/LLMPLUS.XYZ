...
- **Anti-Sycophancy System Prompt**: The system prompt has aggressive anti-silver-lining rules: never reframe defeats as opportunities, never find silver linings without explicit factual grounding, sycophancy is flagged as a critical failure. Court ruling analysis must separate (a) what the court held, (b) what it didn't hold, (c) forward implications — without conflating (b) and (c) as victories. The Tractatus tree update prompt and compression prompt both include matching rules to preserve negative findings with full fidelity during memory updates and compression cycles.
